...
The [`getbestchainlock` RPC](../api/rpc-blockchain.md#getbestchainlock) returns the information about the best ChainLock.
...
The [`getblockchaininfo` RPC](../api/rpc-blockchain.md#getblockchaininfo) provides information about the current state of the block chain.
...
→<br>`bestblockhash` | string (hex) | Required<br>(exactly 1) | The hash of the header of the highest validated block in the best block chain, encoded as hex in RPC byte order.  This is identical to the string returned by the [`getbestblockhash` RPC](../api/rpc-blockchain.md#getbestblockhash)
...
The [`getblockcount` RPC](../api/rpc-blockchain.md#getblockcount) returns the number of blocks in the local best block chain.
...
The [`getblockhash` RPC](../api/rpc-blockchain.md#getblockhash) returns the header hash of a block at the given height in the local best block chain.
...
The [`getblockfilter` RPC](../api/rpc-blockchain.md#getblockfilter) retrieves a [BIP157](https://github.com/bitcoin/bips/blob/master/bip-0157.mediawiki) content filter for a particular block.
...
The [`getblockhashes` RPC](../api/rpc-blockchain.md#getblockhashes) returns array of hashes of blocks within the timestamp range provided (requires `timestampindex` to be enabled).
...
The [`getblockheaders` RPC](../api/rpc-blockchain.md#getblockheaders) returns an array of items with information about the requested number of blockheaders starting from the requested hash.
...
The [`getblockstats` RPC](../api/rpc-blockchain.md#getblockstats) computes per block statistics for a given window.
...
The [`getmerkleblocks` RPC](../api/rpc-blockchain.md#getmerkleblocks) returns an array of hex-encoded merkleblocks for <count> blocks starting from <hash> which match <filter>.
...
The [`getspecialtxes` RPC](../api/rpc-blockchain.md#getspecialtxes) returns an array of special transactions found in the specified block
...
Name | Type | Presence | Description
--- | --- | --- | ---
`result` | string (hex): array of ojbects | Required<br>(exactly 1) | Array of special transaction objects in the format of the [`getrawtransaction` RPC](../api/rpc-raw-transactions.md#getrawtransaction)
...
The [`getspentinfo` RPC](../api/rpc-blockchain.md#getspentinfo) returns the txid and index where an output is spent (requires `spentindex` to be enabled).
...
The [`pruneblockchain` RPC](../api/rpc-blockchain.md#pruneblockchain) prunes the blockchain up to a specified height or timestamp. The `-prune` option needs to be enabled (disabled by default).
...
The [`verifychain` RPC](../api/rpc-blockchain.md#verifychain) verifies each entry in the local block chain database.
...
The [`verifytxoutproof` RPC](../api/rpc-blockchain.md#verifytxoutproof) verifies that a proof points to one or more transactions in a block, returning the transactions the proof commits to and throwing an RPC error if the block is not in our best block chain.
