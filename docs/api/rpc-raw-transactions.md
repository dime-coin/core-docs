...
* [CombinePSBT](../api/rpc-raw-transactions.md#combinepsbt): combines multiple partially-signed Dash transactions into one transaction.
...
* [AnalyzePSBT](../api/rpc-raw-transactions.md#analyzepsbt): analyzes and provides information about the current status of a PSBT and its inputs.
...
The [`decodepsbt` RPC](../api/rpc-raw-transactions.md#decodepsbt) returns a JSON object representing the serialized, base64-encoded partially signed Dash transaction.
...
* [GetSpecialTxes](../api/rpc-blockchain.md#getspecialtxes): returns an array of special transactions found in the specified block
* [GetTransaction](../api/rpc-wallet.md#gettransaction): gets detailed information about an in-wallet transaction.

## GetTxChainlocks

The [`gettxchainlocks` RPC](../api/rpc-raw-transactions.md#gettxchainlocks) returns the block height each transaction was mined at and whether it is ChainLocked or not.
...
The [`joinpsbts` RPC](../api/rpc-raw-transactions.md#joinpsbts) joins multiple distinct PSBTs with different inputs and outputs into one PSBT with inputs and outputs from all of the PSBTs. No input in any of the PSBTs can be in more than one of the PSBTs.
...
| Name         | Type   | Presence             | Description                                                                                                                                                                                                                                                                             |
| ------------ | ------ | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maxfeerate` | number | Optional<br>(0 or 1) | Reject transactions whose fee rate is higher than the specified value, expressed in DASH/kB. Changed from `allowhighfees` in Dash Core 18.0.0. See [previous version](https://dashcore.readme.io/v0.17.0/docs/core-api-ref-rpc-raw-transactions#sendrawtransaction). |
...
The [`utxoupdatepsbt` RPC](../api/rpc-raw-transactions.md#utxoupdatepsbt) updates a PSBT with  data from output descriptors, UTXOs retrieved from the UTXO set or the mempool.
